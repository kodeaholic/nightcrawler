...
import { featureMovieService } from "./feature-movie.service";
const TYPE = {
  published: "PUBLISHED",
  new: "NEW",
};
...
const Table = (props) => {
  const { type } = props;
  const published = type === TYPE.published ? 1 : 0;
  const classBadge = type === TYPE.published ? "badge-success" : "badge-info";
  ...
  useEffect(() => {
    const fetchItems = async () => {
      const res = await featureMovieService.getItems(published, page, limit);
                ...
                {results.map((item, index) => {
                  return (
                    <tr>
                      ...
                      <td className="py-1">
                        <img src={item.posterImg} alt="Thumb" />
                      </td>
                      <td>
